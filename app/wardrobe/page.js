    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-24 text-gray-400">
          <Loader2 className="w-8 h-8 animate-spin mr-2" /> Loading wardrobe…
        </div>
      ) : items.length === 0 ? (
      ...
      {showUpload && (
        <UploadClothingModal onClose={() => setShowUpload(false)} onSuccess={handleUploadSuccess} />
      )}
    </main>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <p className="text-sm text-gray-400 mb-4">
        {hasItems ? "Try adjusting the filters above" : "Start by uploading your first clothing item"}
      </p>
      {!hasItems && (
